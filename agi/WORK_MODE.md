...
Inbox revision 20 clarifies revision 19 as an anti-overgeneralization rule. A negative result from a scientist-agent-derived mechanism or an O-adapted variant is scoped to the candidate, configuration, and conditions actually tested; it is not evidence against the whole scientist-agent family or an untested mechanism. Evaluation must distinguish tested-variant failure, adaptation or ablation loss, original-baseline reproduction failure, and untested mechanisms. When conditions equivalent to an adequately proven original configuration already exist with provenance, reuse that positive control instead of demanding a duplicate reproduction.
