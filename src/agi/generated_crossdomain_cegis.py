...
from continual.acquired_program_tools import AcquiredProgramToolError, acquired_program_candidate_payload
...
from .acquired_programs import ProgramExample, execute_program, synthesize_program, validate_program_descriptor

GENERATED_CROSSDOMAIN_SCOPE_PREFIX = "agi/acquired-program-runtime/generated-crossdomain-cegis-v1"
_GENERATOR_VERSION = "crossdomain-composition-v1"
...
def _seeded_int(seed: str, label: str, low: int, high: int) -> int:
    ...
    raw = hashlib.sha256(
        f"generated-crossdomain-cegis-v1\0{seed}\0{label}".encode("utf-8")
    ).digest()
    return low + int.from_bytes(raw[:8], "big") % (high - low + 1)


def _const_numeric(value: int) -> dict[str, Any]:
...
def _case_node(case_index: int) -> dict[str, Any]:
    op = "upper" if case_index == 0 else "lower"
    return {"op": op, "arg": _input()}


def generated_crossdomain_target(
    seed: str,
) -> tuple[dict[str, Any], dict[str, Any], tuple[Any, ...], tuple[Any, ...], tuple[Any, ...]]:
    """Generate a bounded cross-domain target and disjoint evaluator probes from a seed.

    The seed selects one of four *schemas* but also parameterizes case normalization, multipliers and
    offsets, yielding many exact programs rather than a catalogue of named tasks. Two schemas compose
    three or more typed operators; the sequence-to-string schema combines a variable-length numeric
    fold, arithmetic, and conversion. All generated programs are pure and remain inside the existing
    typed/effect/resource validator.
    """

    family = _seeded_int(seed, "family", 0, 3)
    multiplier = _seeded_int(seed, "multiplier", 2, 3)
    offset = _seeded_int(seed, "offset", -2, 2)
    case_index = _seeded_int(seed, "case", 0, 1)

    if family == 0:
        # string -> string: reverse(case(input)). Initial probes are case-normalized palindromes,
        # making identity a cheaper but wrong hypothesis.
        expression = {"op": "reverse", "arg": _case_node(case_index)}
        input_domain, output_domain = "string", "string"
        initial = ("A", "BB", "CCC") if case_index == 0 else ("a", "bb", "ccc")
        diagnostic = ("Abc", "Model", "xYz", "Reasoning")
        final = ("OpenAI", "Generalization", "aBcDe", "zYxW")
        family_name = "string-case-reverse"
    elif family == 1:
        # string -> sequence: chars(reverse(case(input))). Initial probes make chars(input) look right.
        expression = {
            "op": "chars",
            "arg": {"op": "reverse", "arg": _case_node(case_index)},
        }
        input_domain, output_domain = "string", "sequence"
        initial = ("A", "BB", "CCC") if case_index == 0 else ("a", "bb", "ccc")
        diagnostic = ("AbC", "Model", "xYz", "Transfer")
        final = ("OpenAI", "Persistent", "aBcDe", "qWeRtY")
        family_name = "string-case-reverse-chars"
    elif family == 2:
        # string -> numeric: length(input) * k + b. Equal-length initial strings make a constant
        # hypothesis cheaper; diagnostics reveal the length-dependent affine rule.
        length = {"op": "length_string", "arg": _input()}
        expression = {
            "op": "add",
            "left": {
                "op": "mul",
                "left": length,
                "right": _const_numeric(multiplier),
            },
            "right": _const_numeric(offset),
        }
        input_domain, output_domain = "string", "numeric"
        initial = ("a", "b", "c")
        diagnostic = ("", "ai", "model", "reasoning")
        final = ("tool", "persistent", "generalization", "x" * 17)
        family_name = "string-length-affine"
    else:
        # variable-length sequence -> string: to_string(sum(sequence) * k + b). Initial examples all
        # sum to zero, so a constant looks sufficient until diagnostics force fold+arithmetic+convert.
        folded = {
            "op": "fold_numeric",
            "arg": _input(),
            "reducer": "add",
        ...
        input_domain, output_domain = "sequence", "string"
        initial = ([], [1, -1], [2, -3, 1])
        diagnostic = ([1], [2, 3], [-1, 4], [5, -2, 1, 0], [-3, -2])
        final = ([9, -4], [1, 2, 3, 4], [-5, 2], [7], [3, -1, -1, 8, -4, 2])
        family_name = "sequence-sum-affine-string"
    ...
    if meta["nodes"] > 8:
        raise RuntimeError("generated cross-domain target exceeded the intended bounded grammar")
    return descriptor, {
...
def _measurement(task_id: str, repeat: int, score: float, artifact: Any) -> dict[str, Any]:
    return {
        ...
        "domain": "acquired-program:generated-crossdomain-cegis",
        "repeat_index": repeat,
        "passed": score == 1.0,
        "score": score,
    ...
    *,
    ...
    output = engine._invoke(
        ...
        "execute",
        {
            "snapshot": {"revision": 0},
            "execution_unit": {
                "goal": "apply a regression-verified evaluator-generated cross-domain acquired program",
                "scope": scope,
...
def run_generated_crossdomain_cegis_campaign(
    root: Path,
    seed: str,
    *,
    engine_factory: Callable[[Path], LearningEnabledEngine] = LearningEnabledEngine,
) -> dict[str, Any]:
    """Counterexample-learn a seed-generated cross-domain program with a sealed final holdout."""

    if not isinstance(seed, str) or not seed.strip():
        raise ValueError("seed must be a non-empty string")
    root = root.resolve()
    prior_seed = "generated-crossdomain-protected-prior-v1"
    prior = run_acquired_program_runtime_campaign(root, prior_seed, engine_factory=engine_factory)
    if not prior["passed"]:
        raise RuntimeError("protected prerequisite acquired-program campaign did not pass")

    target, target_meta, initial_inputs, diagnostic_inputs, final_inputs = generated_crossdomain_target(seed)
    ...
    for round_index in range(1, 12):
        ...
        if any(existing.input == counterexample.input for existing in counterexamples):
            raise RuntimeError("cross-domain refinement repeated an already learned probe")
        ...
        raise RuntimeError("cross-domain counterexample refinement exceeded round budget")

    if not counterexamples:
        raise RuntimeError("generated cross-domain target did not falsify the initial hypothesis")
    final_answers = tuple(current.apply(query) for query in final_inputs)
    if final_answers != final_expected:
        _atomic_json(
            root / ".continual" / "evidence" / "generated-crossdomain-cegis-failures" / f"{commitment}.json",
            {
                "schema_version": 1,
                ...
                "claim_boundary": "Internal failed cross-domain generalization evidence only; not independent production AGI evidence.",
            },
        )
        raise RuntimeError("diagnostic-refined cross-domain program failed the sealed final holdout")
    ...
    candidate = acquired_program_candidate_payload(
        ...
        description="Pure bounded program learned from an evaluator-generated committed cross-domain target.",
    )
    candidate["supporting_evidence"] = [
        {
            "type": "generated_crossdomain_evaluator_commitment",
            "generator_version": _GENERATOR_VERSION,
            "commitment_sha256": commitment,
            "seed_sha256": _digest(seed),
            "target_program_sha256": _digest(target),
    ...
    _atomic_json(
        candidate_dir / "learning-history" / "generated-crossdomain-cegis.json",
        {
            "schema_version": 1,
            "generator_version": _GENERATOR_VERSION,
            "commitment_sha256": commitment,
    ...
    target_task = f"withheld-generated-crossdomain-{token}"
    for repeat, (query, expected, answer) in enumerate(
        ...
        baseline.append(_measurement(target_task, repeat, 1.0 if pre_solved else 0.0, {**artifact, "phase": "baseline"}))
    ...
    prior_before = _prior_retest(root, prior_seed, prior, engine_factory=engine_factory)
    protected = _measurement(
        "protected-prior-generated-crossdomain",
        0,
        prior_before["score"],
        {
    ...
    _atomic_json(adverse_path, [*candidate_measurements, adverse_protected])
    _atomic_json(valid_path, [*candidate_measurements, protected])
    ...
    if failed["decision"]["adopt_candidate"]:
        raise RuntimeError("cross-domain Candidate ignored a forced protected regression")
    ...
    if not promoted["decision"]["adopt_candidate"]:
        raise RuntimeError("cross-domain Candidate failed valid deterministic regression")
    ...
    persisted_history = json.loads(
        (candidate_dir / "learning-history" / "generated-crossdomain-cegis.json").read_text(encoding="utf-8")
    )
    learner_pairs = {
        (_digest(item.input), _digest(item.output)) for item in [*initial_examples, *counterexamples]
    }
    final_leaked = any(
    ...
    report = {
        ...
        "generator_version": _GENERATOR_VERSION,
        "seed_sha256": _digest(seed),
        "family_index": target_meta["family_index"],
        "family": target_meta["family"],
        "input_domain": target["input_domain"],
        "output_domain": target["output_domain"],
        "target_program_sha256": _digest(target),
        ...
        "target_descriptor_persisted_to_learner": bool(persisted_history["target_descriptor_persisted"]),
        ...
        "claim_boundary": (
            "This is internally evaluator-generated cross-domain capability-development evidence. "
            "It is not independent production evidence and does not support a verified AGI claim."
        ),
    }
    report["passed"] = bool(
    ...
    _atomic_json(
        root / ".continual" / "evidence" / "generated-crossdomain-cegis" / f"{report['campaign_id']}.json",
        report,
    )
    return report
