...
def test_three_crash_recovered_generated_rounds_remain_functionally_replayable(
    runtime_repo: Path,
) -> None:
    """Falsify whether the retained-growth path survives a third distinct generated round.

    This extends the existing two-round functional-retention boundary without changing the
    learner, generator, evaluator, or gates. A third seed must add a third distinct verified
    Candidate, preserve all previously committed Candidate/trial bytes, and a fresh state-only
    resolver must rediscover and execute all three skills without caller Candidate IDs while at
    least one unrelated generated gap remains fail-closed.
    """
    campaign_id = "generated-cross-round-three-round-retention-test"
    seeds = (
    ...
    assert len(report["learned_candidate_ids"]) == 3
    assert len(set(report["learned_candidate_ids"])) == 3
    ...
    assert len(report["fresh_replay"]) == 3
