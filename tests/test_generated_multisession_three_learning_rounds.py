...
def test_three_generated_learning_sessions_retain_all_skills_state_only(
    runtime_repo: Path,
) -> None:
    report = run_generated_multisession_functional_retention(
        runtime_repo,
        "generated-multisession-three-learning-test",
        seeds=(
    ...
    assert report["learning_session_count"] == 3
    assert report["state_only_restart_count"] == 4
    ...
    assert len(report["fresh_session_replays"]) == 3
