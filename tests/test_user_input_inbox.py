...
def test_unapplied_user_inputs_are_revision_ordered() -> None:
    ...
    assert [entry["sequence"] for entry in unapplied_user_inputs(inbox, after_revision=0)] == [1, 2]
    assert [entry["sequence"] for entry in unapplied_user_inputs(inbox, after_revision=1)] == [2]
    assert unapplied_user_inputs(inbox, after_revision=2) == []
