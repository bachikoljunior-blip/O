...
def test_five_crash_recovered_generated_rounds_remain_functionally_replayable(
    runtime_repo: Path,
) -> None:
    """Falsify whether retained generated-skill growth survives a fifth distinct round."""
    campaign_id = "generated-cross-round-five-round-retention-test"
    seeds = (
        ...
        "multi-round-generated-goal-recovery-seed:round-5",
    )
    ...
    assert len(report["learned_candidate_ids"]) == 5
    assert len(set(report["learned_candidate_ids"])) == 5
    ...
    assert len(report["fresh_replay"]) == 5
