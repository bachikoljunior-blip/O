...
16. Use `agi/HOURLY_EXECUTION_STATE.json` as the single-execution lease and progress record. Never run two hourly executions concurrently. Resolve the assigned run with platform scheduled metadata when available, otherwise with `src/agi/hourly_window.py`; never discard a trigger within two minutes before an hourly boundary as an expired previous-hour run. Finish or safely checkpoint by the resolved policy hard stop before the next hourly boundary.
