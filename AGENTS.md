...
13. For a scheduled standalone request whose first line is `agiを作って`, read `agi/STANDALONE_HOURLY_AUTOMATION.md`, `agi/REPORTING_TARGET.json`, `agi/CONTINUATION.json`, `agi/AUTONOMY_STATE.json`, and `agi/HOURLY_EXECUTION_STATE.json` before selecting or executing work.
...
15. Every hourly session starts with zero prior chat/session context. Repository state is durable memory; claims made by a previous session are not evidence unless the repository contains the supporting artifact. Sending a final report to the control chat is output delivery only and must not be used to read or import that chat's prior context.
...
18. The primary destination for every hourly report is the existing control chat identified by `agi/REPORTING_TARGET.json`, not the newly created standalone execution chat. Use a platform-provided parent/control-chat reply route when one is actually exposed. Never invent a conversation identifier or claim delivery without platform confirmation.
19. If direct control-chat delivery is unavailable, persist the complete factual report under `agi/reports/` and update `agi/LATEST_HOURLY_REPORT.json` before releasing the lease. Then post a fallback notice in the standalone execution chat naming the report artifact and stating that direct control-chat delivery was not confirmed. The fallback must not be described as delivery to the control chat.
20. Every report must include its time window, actual model, completed changes, PRs and SHAs, successful and failed validation, unmerged or blocked work, exact next action, and truthful AGI claim boundary. This is required even when the run does no work because of overlap, permissions, model mismatch, failure, or interruption.
21. Finalization order is strict: stop starting work; finish or abandon in-flight mutations safely; inspect latest main and final CI/PR state; persist continuation and the exact report artifact; release the lease; attempt delivery to the control chat; use the declared fallback only when necessary. After the lease is marked completed/interrupted, do not merge, commit, update a PR, or otherwise mutate the repository. Late results remain pending for the next fresh session to reconcile.
